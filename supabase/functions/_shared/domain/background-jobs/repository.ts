...
export interface BackgroundJobItemRecord {
  id: string
  job_id: string
  user_id: string
  source_table: string | null
...
export async function upsertBackgroundJob(
  ...
  if (error || !data) {
    throw error ?? new Error('Falha ao upsert de background job')
  }

  return data as BackgroundJobRecord
}

export async function createBackgroundJobItems(
  supabase: AppSupabaseClient,
  items: CreateBackgroundJobItemInput[],
...
export async function updateBackgroundJobItem(
  supabase: AppSupabaseClient,
  itemId: string,
  updates: BackgroundJobItemUpdateInput,
): Promise<void> {
  const payload = {
    ...updates,
    updated_at: updates.updated_at ?? new Date().toISOString(),
...
export async function appendBackgroundJobEvent(
  supabase: AppSupabaseClient,
  event: BackgroundJobEventInput,
): Promise<void> {
  const { error } = await supabase
