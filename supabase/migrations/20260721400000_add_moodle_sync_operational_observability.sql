-- Aggregated, service-only operational telemetry for the durable Moodle
-- worker.  The projection deliberately contains no Moodle identifier,
-- credentials, response payload, aliases or error text.

CREATE INDEX IF NOT EXISTS idx_background_jobs_moodle_observability
  ON public.background_jobs (completed_at DESC, started_at DESC, created_at DESC)
  WHERE job_type = 'moodle_sync' AND source = 'sync';

CREATE INDEX IF NOT EXISTS idx_background_job_items_moodle_stuck
  ON public.background_job_items (leased_until, heartbeat_at, updated_at)
  WHERE status = 'processing';

CREATE OR REPLACE FUNCTION public.backend_get_moodle_sync_operational_metrics(
...
COMMENT ON FUNCTION public.backend_get_moodle_sync_operational_metrics(INTEGER, INTEGER) IS
  'Service-only sanitized operational aggregates per Moodle site and connection; excludes credentials, Moodle identities, payloads and error text.';
