...
describe('risk.service', () => {
  ...
  it('retries when course-level risk update hits a deadlock', async () => {
    rpcMock
      .mockResolvedValueOnce({
        data: null,
        error: {
  ...
  it('retries when student-level fallback hits a deadlock', async () => {
    rpcMock
      .mockResolvedValueOnce({
        data: null,
        error: {
    ...
    expect(rpcMock).toHaveBeenNthCalledWith(4, 'update_student_risk', {
      p_student_id: 'student-2',
    });
    expect(result).toEqual({
      failedCount: 0,
